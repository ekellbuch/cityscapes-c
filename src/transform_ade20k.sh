...
ZIPPED_FOLDER_NAME="ADEChallengeData2016"
...
CITYSCAPES_DIR="${EXTRACTED_DIR}/${ZIPPED_FOLDER_NAME}/${UNZIPPED_FOLDER_NAME}/validation"
...
do
  ...
  rm -rf $ADE20K_DIR"_"$corruption"-"$severity"/images/training"
  rm -rf $ADE20K_DIR"_"$corruption"-"$severity"/annotations/training"
  # remove validation data
  rm -rf $ADE20K_DIR"_"$corruption"-"$severity"/images/validation"

  # for the validation data, update the images
  DATA_IN="${ADE20K_DIR}/images/validation"
  DATA_OUT="${ADE20K_DIR}_${corruption}-${severity}/images"
